...
function test_request_mysql(){
    host=$1
    port=$2
    db=$3
    result=false
    echo "Making mysql connection to '$db' at '$host:$port' ..."
    mysql -uroot -ppassword -h $host -p $port -e "USE $db;" | grep -q "Database changed" && result=true
    assert_result ${result}
}
...
service_info ${service}
test_container_is_running ${service}
test_request_mysql 127.0.0.1 3306 dev
